...
export const userService = {
  ...
  deleteUser: async (userId: number) => {
    const response = await apiClient.delete(`/api/user/DeleteUsers?userid=${userId}`);
    return response.data;
  },
  getUserRoles: async () => {
    const response = await apiClient.get('/api/user/GetRoles');
    return response.data;
  },
  ...
  getUserProfile: async (userId: number)=>{
    ...
    const response = await apiClient.get(`/api/user/GetUserProfile?userId=${userId}`);
    return response.data;
  } 
};